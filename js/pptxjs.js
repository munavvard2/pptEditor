    ...
    $.fn.pptxToHtml = function (options) {
        ...
        function genShape(node, pNode, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj, isUserDrawnBg, sType, source) {
            ...
            if (shapType !== undefined && custShapType === undefined) {
                ...
                result += "<div class='block draggable " + getVerticalAlign(node, slideLayoutSpNode, slideMasterSpNode, type) + //block content
        ...
        */
        function processPicNode(node, warpObj, source, sType) {
            ...
            rtrnData = "<div class='block content draggable' style='" +
        ...
        function genTextBody(textBodyNode, spNode, slideLayoutSpNode, slideMasterSpNode, type, idx, warpObj, tbl_col_width) {
            ...
            for (var i = 0; i < apNode.length; i++) {
                ...
                if (rNode === undefined && pNode !== undefined) {
                    ...
                    if (isBullate) {
                        var txt_obj = $(prgr_text)
                            .css({ 'position': 'relative', 'float': 'left', 'white-space': 'nowrap', 'visibility': 'hidden' })
                            .appendTo($('body'));
                ...
                text += "<div class='text-block' style='height: 100%;direction: initial;overflow-wrap:break-word;word-wrap: break-word;" + prg_width + margin + "' >";
                text += prgrph_text;
                text += "</div>";
                text += "</div>";
            }
        ...
        function genSpanElement(node, rIndex, pNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, rNodeLength, warpObj, isBullate) {
            ...
            if (linkID !== undefined && linkID != "") {
                ...
                return openElemnt + " class='" + cssName + "' style='" + text_style + "'><a href='" + linkURL + "' " + linkColorSyle + "  " + linkTooltip + " target='_blank'>" +
                        text.replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;').replace(/\s/g, "&nbsp;") + "</a>" + closeElemnt;
            } else {
                return openElemnt + " class='" + cssName + "' style='" + text_style + "'>" + text.replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;').replace(/\s/g, "&nbsp;") + closeElemnt;//"</bdi>";
            }
        ...
        function genChart(node, warpObj) {
            ...
            var order = node["attrs"]["order"];
            var xfrmNode = getTextByPathList(node, ["p:xfrm"]);
            var result = "<div id='chart" + chartID + "' class='block content draggable' style='" +
